...
//				|| type == "mov" || type == "m4v"
		...
		//TODO improve IF statement check for RiseCache v2
		if (url.indexOf('http://localhost:9494/files?url=') != -1) {
			if (url.lastIndexOf(".") != -1) {
				type = url.substring(url.lastIndexOf(".") + 1);
				if (type.indexOf(encodeURIComponent("?")) != -1) {
					type = type.substring(0, type.indexOf(encodeURIComponent("?")));
				}
				if (type == "mov") {
					type = "mp4";
				}
