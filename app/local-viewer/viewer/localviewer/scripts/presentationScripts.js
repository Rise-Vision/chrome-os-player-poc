...
function gadgetRequestAttempt(id, xmlUrl, attempt) {
	if (xmlUrl != null && attempt < 4) {
		...
		$(document).oneTime && $(document).oneTime("60s", "t" + id, function() {
...
function itemLoaded(id) {
	try {
		...
		$(document).stopTime && $(document).stopTime("t" + id);
...
function playCmd(id) {
    try {   
        ...
        var myFrame = document.getElementById("if_" + id);
        myFrame.contentWindow.embedPlay && myFrame.contentWindow.embedPlay();
    	}
...
function pauseCmd(id) {
    try { 
    	if (id.charAt(id.length - 1) == "p") {
        var myFrame = document.getElementById("if_" + id);
        myFrame.contentWindow.embedPause && myFrame.contentWindow.embedPause();
    	}
    	else if (id.charAt(id.length - 1) == "v") {
    		$('#' + id).css('z-index', 1);
...
function stopCmd(id) {
    try { 
    	if (id.charAt(id.length - 1) == "p") {
        var myFrame = document.getElementById("if_" + id);
        myFrame.contentWindow.embedStop && myFrame.contentWindow.embedStop();
    	}
    	else if (id.charAt(id.length - 1) == "v") {
    		$('#' + id).css('z-index', 1);
    	    document.getElementById("if_" + id).contentWindow.stop();
...
function renderGadgetAttemptOld(html, htmlName, attempt) {
	if (html != null && attempt < 4) {
		...
		$(document).oneTime && $(document).oneTime("60s", "t" + htmlName, function() {
...
function updateWidget(url, containerName, htmlName, transition) {
  try {   	
    if (htmlName != null) {
      ...
      renderWidgetAttempt(url, htmlName, 1);
    }
  }
  catch (err) {
...
function renderWidgetAttempt(url, htmlName, attempt) {
  if (url && attempt < 31) {
    var myContainer = getElement(htmlName);
    if (myContainer != null) {
    	// clear Widget div prior to widget being requested again
    	myContainer.innerHTML = '';
    }
    ...
    parent.writeToLog("Widget ID=" + htmlName + " is loading (" + attempt + ").");

    $(document).oneTime && $(document).oneTime("60s", "t" + htmlName, function() {
      parent.window.RiseVision.Viewer.ExtLogger.logViewerEvent('Widget Failed to Load (' + attempt + ')', url);

      renderWidgetAttempt(url, htmlName, attempt + 1);
    });
  }
  else {
    parent.writeToLog("Widget ID=" + htmlName + " failed to load.");
  }
}
