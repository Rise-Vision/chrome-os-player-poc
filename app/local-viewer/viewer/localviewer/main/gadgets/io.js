...
//    var UNPARSEABLE_CRUFT = config['unparseableCruft'];
  ...
  /**
   * Checks to see if Viewer is running in any environment other than Player
   *
   * @return {boolean} true if 'localhost, 'preview', or 'rvaviewer-test' is found within the host value of document.location
   */
  function isTestServer() {
    return document.location.host.indexOf( "localhost" ) > -1 ||
      document.location.host.indexOf( "preview" ) > -1 ||
      document.location.host.indexOf( "rvaviewer-test" ) > -1;
  }
...
//      var proxyUrl = config['jsonProxyUrl'].replace('%host%', document.location.host);
      var proxyHost = isTestServer()  ? "//" + document.location.host : "http://rvashow.appspot.com";

      var proxyUrl = proxyHost + "/gadgets/makeRequest";
