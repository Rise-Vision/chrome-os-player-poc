...
(function(jQuery,undefined){
	...
	jQuery.fn.extend({
		...
		revkill: function(oy) {
              ...
              // Donna Start - Destroy arrows.
              container.parent().find('.tp-leftarrow, .tp-rightarrow').each(function() {
                var el = jQuery(this);
		...
		*******************************************************/
		...
		var prepareSlides = function(container,opt) {
			...
			container.find('>ul:first >li >img').each(function(j) {
				...
				img.replaceWith('<div class="tp-bgimg defaultimg" data-lazyload="'+img.data('lazyload')+
          '" data-bgfit="'+bgfit+'"data-bgposition="'+bgposition+'" data-bgrepeat="'+bgrepeat+
          '" data-lazydone="'+img.data('lazydone')+'" src="'+src+'" data-src="'+src+
          '" style="background-color:'+img.css("backgroundColor")+';background-repeat:'+
          bgrepeat+';background-image:url(\''+src.replace("'", "\\'")+'\');background-size:'+
          bgfit+';background-position:'+bgposition+';width:100%;height:100%;"></div>');
										...
										nextcaption.find('iframe').each(function() {
												...
												if (nextcaption.data('videoposter')!=undefined && nextcaption.data('videoposter').length>2 && nextcaption.data('autoplay')!=true && !internrecalled) {
													if (nextcaption.find('.tp-thumb-image').length==0)
														nextcaption.append('<div class="tp-thumb-image" style="cursor:pointer; position:absolute;top:0px;left:0px;width:100%;height:100%;background-image:url('+
                              nextcaption.data('videoposter')+'); background-size:cover"></div>');
													else
		...
		**************************************************************************/
		...
		var createThumbs = function(container,opt) {
			...
			container.find('>ul:first >li').each(function(i) {
							...
							bullets.append('<div class="bullet thumb" style="background-color:'+bgcolor+
                ';position:relative;width:'+opt.thumbWidth+'px;height:'+opt.thumbHeight+
                'px;background-image:url('+src+
